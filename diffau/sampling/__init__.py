...
def get_pc_sampler(
    ...
    denoise=True, eps=3e-2, snr=0.1, corrector_steps=1, probability_flow: bool = False,num_high_order_channels= 12,
    ...
    def pc_sampler():
        ...
        with torch.no_grad():
            B,C,F,T = y.shape
            init = y[:,0].clone().unsqueeze(0).repeat(1,num_high_order_channels,1,1) #for OUVE start with omni channel, for VE/FM this is ignored
            xt = sde.prior_sampling(init.shape, init)
            ...
            for i in range(sde.N):
                ...
                xt, xt_mean = predictor.update_fn(xt, y, vec_t, stepsize) #x_t is noisy high order channels, y is low order channels
...
def get_flow_matching_sampler(sde, velocity_fn, y, N=50, method='euler',num_high_order_channels = 12, eps=3e-2, **kwargs):
    """Flow Matching ODE sampler."""
    
    def flow_sampler():
        with torch.no_grad():
            init = y[:,0].clone().unsqueeze(0).repeat(1,num_high_order_channels,1,1) #just to get the shape right, actual values are ignored for flow matching prior sampling
            x = sde.prior_sampling(init,y).to(y.device)
            ...
            for i in range(N):
                ...
                if i != len(timesteps) - 1:
                    ...
                    if  method == 'euler' or  method == 'midpoint':
                        stepsize = timesteps[-1] # from eps to 0
                    else:
                        stepsize = timesteps[-1] - eps
                
                vec_t = torch.ones(y.shape[0], device=y.device) * t

                if method == 'euler':
                    v_t = velocity_fn(x, y, vec_t)
                    x = x + stepsize * v_t
                    
                elif method == 'midpoint':
                    k1 = velocity_fn(x, y, vec_t)
                    x_mid = x + (stepsize / 2) * k1
                    vec_t_mid = torch.ones(y.shape[0], device=y.device) * (t - stepsize / 2)
                    k2 = velocity_fn(x_mid, y, vec_t_mid)
                    x = x + stepsize * k2
                    
                elif method == 'heun':
                    k1 = velocity_fn(x, y, vec_t)
                    x_pred = x + stepsize * k1
                    vec_t_next = torch.ones(y.shape[0], device=y.device) * (t - stepsize)
                    k2 = velocity_fn(x_pred, y, vec_t_next)
                    x = x + (stepsize / 2) * (k1 + k2)
                    
                elif method == 'rk4':
                    k1 = velocity_fn(x, y, vec_t)
                    
                    x_2 = x + (stepsize / 2) * k1
                    vec_t_2 = torch.ones(y.shape[0], device=y.device) * (t - stepsize / 2)
                    k2 = velocity_fn(x_2, y, vec_t_2)
                    
                    x_3 = x + (stepsize / 2) * k2
                    k3 = velocity_fn(x_3, y, vec_t_2)
                    
                    x_4 = x + stepsize * k3
                    vec_t_4 = torch.ones(y.shape[0], device=y.device) * (t - stepsize)
                    k4 = velocity_fn(x_4, y, vec_t_4)
